...
<nav class="fixed top-0 left-0 right-0 bg-white/95 backdrop-blur-sm shadow-sm z-50">
        ...
        <div class="flex items-center justify-between h-20">
            ...
            <div class="flex items-center space-x-4">
                @if(session('user'))
                    {{-- Jika sudah login --}}
                    <div class="flex items-center space-x-4">
                        <div class="flex items-center space-x-2 bg-green-400 px-4 py-2 rounded-lg">
                            <div class="w-8 h-8 bg-ecobank-green rounded-full flex items-center justify-center">
                                <span class="text-white font-semibold text-sm">{{ strtoupper(substr(session('user.username'), 0, 1)) }}</span>
                            </div>
                            <span class="text-gray-700 font-medium">{{ session('user.username') }}</span>
                        </div>
                        
                        @if(session('user.role') === 'admin')
                            <a href="{{ route('admin.dashboard') }}" class="bg-ecobank-yellow text-gray-900 px-6 py-2 rounded-lg font-semibold hover:bg-yellow-500 transition shadow-md">
                                Dashboard
                            </a>
                        @else
                            <a href="{{ route('user.dashboard') }}" class="bg-ecobank-yellow text-gray-900 px-6 py-2 rounded-lg font-semibold hover:bg-yellow-500 transition shadow-md">
                                Dashboard
                            </a>
                        @endif
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="text-gray-700 hover:text-red-600 transition font-medium flex items-center">
                ...
                @else
                    {{-- Jika belum login --}}
                    <a href="{{ route('login') }}" class="bg-ecobank-yellow text-gray-900 px-6 py-2 rounded-lg font-semibold hover:bg-yellow-500 transition shadow-md">
                        Login
                    </a>
                    <a href="{{ route('register') }}" class="bg-ecobank-yellow text-gray-900 px-6 py-2 rounded-lg font-semibold hover:bg-yellow-500 transition shadow-md">
                        Daftar
                    </a>
                @endif
            </div>
   ...
   {{-- About Section --}}
<section id="about" class="py-24 bg-ecobank-light">
    <div class="max-w-6xl mx-auto px-6 text-center">
        ...
        {{-- Cards 1 & 2 --}}
        <div class="grid md:grid-cols-2 gap-10 mb-20 place-items-center">
            
            {{-- Card 1 --}}
            <div class="bg-white p-10 rounded-2xl shadow-lg hover:shadow-2xl transition text-center max-w-md">
                <div class="w-16 h-16 bg-ecobank-yellow rounded-full flex items-center justify-center mx-auto mb-6">
            ...
            {{-- Card 2 --}}
            <div class="bg-white p-10 rounded-2xl shadow-lg hover:shadow-2xl transition text-center max-w-md">
                <div class="w-16 h-16 bg-ecobank-green rounded-full flex items-center justify-center mx-auto mb-6">
                    <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        {{-- Large Card --}}
        <div class="bg-white p-12 rounded-2xl shadow-lg max-w-5xl mx-auto">
...
<section id="contact" class="py-20 bg-white">
    <div class="max-w-4xl mx-auto px-6 lg:px-8 text-center">
       ...
       <button onclick="openContact()" class="bg-green-600 text-white px-12 py-4 rounded-lg font-bold text-xl hover:bg-green-700 transition shadow-lg">
    Contact Us
</button>
    </div>
</section>
{{-- Modal Contact --}}
<div id="contactModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-[999]">
    <div class="bg-white rounded-xl shadow-xl w-[90%] md:w-[500px] p-8 relative animate-fade">
        <button onclick="closeContact()" class="absolute top-3 right-3 text-gray-500 hover:text-red-500 text-2xl">&times;</button>
        <h3 class="text-3xl font-bold text-ecobank-green text-center mb-6">Hubungi Kami</h3>
        ...
        <form action="{{ route('contact.save') }}" method="POST">
            ...
            <div class="mb-4 text-left">
                ...
                <input type="text" name="nama" class="w-full mt-1 border rounded-lg px-4 py-2 focus:ring-2 focus:ring-ecobank-yellow" required>
            </div>

            <div class="mb-4 text-left">
                <label class="font-semibold text-gray-700">Email</label>
                <input type="email" name="email" class="w-full mt-1 border rounded-lg px-4 py-2 focus:ring-2 focus:ring-ecobank-yellow" required>
            </div>

            <div class="mb-6 text-left">
                <label class="font-semibold text-gray-700">Pesan</label>
                <textarea name="pesan" rows="4" class="w-full mt-1 border rounded-lg px-4 py-2 focus:ring-2 focus:ring-ecobank-yellow" required></textarea>
            </div>

            <button type="submit" class="w-full bg-green-500 text-white py-3 rounded-lg font-bold text-lg hover:bg-green-600 transition">
            Kirim
            </button>
